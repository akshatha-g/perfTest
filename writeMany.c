...
int main(int argc, char **argv) {
    if ( argc != 3) {
        printf("Usage: ./writeMany <mnt_directory> <num_of_threads>\n");
        exit(1);
    }
...
#pragma omp barrier
        ...
        bool success = write_sequential(&state);
        if (!success) {
...
#pragma omp barrier
        ...
        if (tid == 0) {
            write_data = sb.st_size * FILE_COUNT;
        } 

    }  /* All threads join master thread and terminate */

    double val = (write_data * nthreads * (CPU_FREQ * 1000000) ) / (  (end - start) * 1024 * 1024 * 1024); // GB/sec
    printf("%lf\n", val);
